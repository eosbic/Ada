...
from api.services.graph_navigator import get_report_links as graph_get_links
...
async def get_report_links(report_id: str, empresa_id: str = Query(...)):
    """Obtener todos los reportes enlazados a un reporte especifico."""
    links = graph_get_links(report_id, empresa_id)
    return {"report_id": report_id, "links": links, "total": len(links)}


@router.get("/reports/by-tag")
async def reports_by_tag(
    empresa_id: str,
    tag: str,
    limit: int = 20,
    db: AsyncSession = Depends(get_db),
):
    """Busca reportes por tag. Ej: /reports/by-tag?empresa_id=xxx&tag=margen-negativo"""
    result = await db.execute(
        text("""
            SELECT id, title, source_file, report_type, tags, alerts, created_at
            FROM ada_reports
            ...
            LIMIT :limit
        """),
        {"eid": empresa_id, "tag": tag, "limit": limit},
    )
    rows = result.fetchall()
    return {
        "tag": tag,
