...
-- =============================================================================
-- MIGRACIÓN: Habilidades Obsidian en Ada V5
-- Ejecutar una sola vez en PostgreSQL
-- Fecha: 2026-03-16
-- =============================================================================

-- 1. EXTENSIÓN FUZZY SEARCH (pg_trgm)
-- Habilita búsqueda tolerante a typos via similitud de trigramas
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- 2. COLUMNAS NUEVAS EN ada_reports
-- Tags semánticos automáticos
ALTER TABLE ada_reports
...
ALTER TABLE ada_reports
    ADD COLUMN IF NOT EXISTS parent_report_id UUID
        REFERENCES ada_reports(id) ON DELETE SET NULL;

-- 3. TABLA report_links (si no existe)
CREATE TABLE IF NOT EXISTS report_links (
    source_report_id UUID NOT NULL REFERENCES ada_reports(id) ON DELETE CASCADE,
    target_report_id UUID NOT NULL REFERENCES ada_reports(id) ON DELETE CASCADE,
...
-- Tags: búsqueda por tag individual con ANY()
CREATE INDEX IF NOT EXISTS idx_reports_tags
    ON ada_reports USING GIN (tags);

-- Fuzzy search en título y nombre de archivo
...
-- Versionado: buscar versiones anteriores del mismo archivo
CREATE INDEX IF NOT EXISTS idx_reports_source_version
    ON ada_reports (empresa_id, source_file, version DESC);

-- report_links: navegación bidireccional eficiente
CREATE INDEX IF NOT EXISTS idx_report_links_source
    ON report_links (source_report_id);

CREATE INDEX IF NOT EXISTS idx_report_links_target
    ON report_links (target_report_id);

-- 5. VERIFICACIÓN
DO $$
BEGIN
    RAISE NOTICE '=== MIGRACIÓN OBSIDIAN COMPLETA ===';
    RAISE NOTICE 'pg_trgm: OK';
    RAISE NOTICE 'tags[]: OK';
    RAISE NOTICE 'version + parent_report_id: OK';
    RAISE NOTICE 'report_links: OK';
    RAISE NOTICE 'Índices GIN trgm + tags + version: OK';
END $$;