...
# Estado de aprobaciones pendientes por usuario: {telegram_id: {draft_id, to, subject, empresa_id}}
_PENDING_APPROVALS: dict = {}

APPROVAL_KEYWORDS = {"sí", "si", "ok", "enviar", "confirmar", "envíalo", "envialo", "dale", "yes"}
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── HITL: interceptar aprobacion de email pendiente ──────────
    msg_clean = message.strip().lower()
    if telegram_id in _PENDING_APPROVALS and msg_clean in APPROVAL_KEYWORDS:
        pending = _PENDING_APPROVALS.pop(telegram_id)
        draft_id = pending.get("draft_id", "")
        empresa_id = pending.get("empresa_id", user_data["empresa_id"])
        if draft_id:
            try:
                async with httpx.AsyncClient(timeout=30) as client:
                    resp = await client.post(
                        f"{API_URL}/chat/chat",
                        json={
                            "message": f"enviar draft_id:{draft_id}",
                            "empresa_id": empresa_id,
                ...
                data = resp.json()
                await update.message.reply_text(data.get("response", "Email enviado."))
            except Exception as e:
                print(f"HITL send error: {e}")
                await update.message.reply_text("Error enviando el email. Intenta de nuevo.")
        else:
            await update.message.reply_text("No encontre el borrador pendiente.")
        return

    if telegram_id in _PENDING_APPROVALS and msg_clean in {"no", "cancelar", "cancel"}:
        _PENDING_APPROVALS.pop(telegram_id)
    ...
    try:
        ...
        # Detectar borrador de email pendiente de aprobacion
        if data.get("needs_approval") and data.get("draft_id"):
            _PENDING_APPROVALS[telegram_id] = {
                "draft_id": data["draft_id"],
                "empresa_id": user_data["empresa_id"],
            }
            print(f"HITL: Borrador pendiente guardado para {telegram_id} → draft_id={data['draft_id']}")
        elif "¿Lo envío?" in response_text or "Responde 'sí'" in response_text:
            # Fallback: el agente puso el draft_id en el texto pero no en el campo
            import re as _re
            draft_match = _re.search(r'draft[_\s]?id[:\s]+([A-Za-z0-9_-]+)', response_text, _re.I)
            if draft_match:
                _PENDING_APPROVALS[telegram_id] = {
